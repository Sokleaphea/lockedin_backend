...
export const createGroupChat = async (req: Request, res: Response) => {
    try {
        ...
        for (const id of uniqueMembers) {
            ...
            if (follow1 && follow2) validMembers.push(id);
        }
        ...
        const channelId = buildChannelId(group._id);
...
export const addGroupMembers = async (req: Request, res: Response) => {
    try {
        ...
        if (added.length > 0) {
            const users = await User.find({ _id: { $in: added } })
                .select("username displayName avatar");
...
export const leaveGroup = async (req: Request, res: Response) => {
    try {
        ...
        const members = group.memberIds.map((m: any) => m.toString());

        if (!members.includes(userId)) {
            return res.status(400).json({ message: "Not a member" });
        }

        if (group.ownerId.toString() === userId && members.length > 1) {
